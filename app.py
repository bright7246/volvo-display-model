...
st.markdown(
    ...
    div.volvo-segment-row div.stButton > button[kind="secondary"] {{ background-color: transparent !important; color: #727a85 !important; border: none !important; border-radius: 22px !important; height: 40px !important; box-shadow: none !important; box-shadow: none !important; }}
...
# 🔊 [설정 -> 사운드] 메인 페이지
    ...
    with st.container(border=True):
        ...
        with sr_col1: st.markdown('<div class="setting-title" style="padding-top: 4px;">서라운드</div>', unsafe_allow_html=True)
        with sr_col2:
...
# 💻 [설정 -> 시스템] 메인 리스트 탭
    ...
    with c3: st.markdown(f'<div class="text-container-fix"><div class="system-item-main">날짜 및 시간</div></div>', unsafe_allow_html=True)
    with c4:
        if st.button("〉", key="main_sys_time", use_container_width=True): st.session_state.sub_page = "sys_datetime"; st.rerun()
    st.markdown('<div style="border-bottom: 1px solid #232830; margin: 8px 0;"></div>', unsafe_allow_html=True)
...
# 📱 [시스템 -> 상세 3. 애플리케이션 (기본 앱 목록)] 상세페이지 (2번 이미지 완벽 이식 + 모든 앱 보기 추가)
...
# 🔍 [시스템 -> 애플리케이션 -> NUGU Auto 앱 정보] 세부 페이지 (⚠️ 무작정 with 처리하던 코드 정상 교체 완료)
    ...
    with st.container(border=True):
        ...
        with col_st2: 
            # 🛠️ [에러 원인 해결] with 블록 선언 버그를 일반 조건형 st.button 처리로 깔끔하게 원복
